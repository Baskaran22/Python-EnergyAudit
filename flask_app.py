...
def sql_database():
    ...
    #results = sql_query(''' SELECT * FROM energy''')
    #msg = 'SELECT * FROM energy'
    results = sql_query(''' SELECT * FROM energy''')
    msg = 'SELECT * FROM energy'
    return render_template('energy.html', results=results, msg=msg)
...
def sql_datainsert():
    ...
    if request.method == 'POST':
        ...
        heatercoolerusage = request.form['heatercoolerusage']
        lightfanusage = request.form['lightfanusage']
        refrigeratorusage = request.form['refrigeratorusage']
        waterheaterusage = request.form['waterheaterusage']
        washerdryerusage = request.form['washerdryerusage']
        ...
        sql_edit_insert(''' INSERT INTO energy (firstname,lastname,address, heatercoolerusage, lightfanusage, refrigeratorusage, waterheaterusage, washerdryerusage) VALUES (?,?,?,?,?,?,?,?) ''',(firstname,lastname,address,heatercoolerusage,lightfanusage,refrigeratorusage,waterheaterusage,washerdryerusage) )
        results = sql_query(''' SELECT * FROM energy''')
        msg = 'INSERT INTO energy (firstname,lastname,address,heatercoolerusage, lightfanusage, refrigeratorusage, waterheaterusage, washerdryerusage) VALUES ('+firstname+','+lastname+','+address+','+heatercoolerusage+','+lightfanusage+','+refrigeratorusage+','+waterheaterusage+','+washerdryerusage+')'
    return render_template('energy.html', results=results, msg=msg)
...
def sql_editlink():
    ...
    results = sql_query(''' SELECT * FROM data_table''')
    return render_template('energy.html', eresults=eresults, results=results)
